...
def save_keras_model(model,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        tf.keras.models.save_model(model, temp_dir,
                                   include_optimizer=include_optimizer)
        # Recursively copy SavedModel dir into target path
        gfile.makedirs(path)
        for root, _, files in os.walk(temp_dir):
            for fname in files:
                ...
                gfile.makedirs(os.path.dirname(dst))
...
def save_keras_model_as_h5(model,
                           filename: Text,
                           include_optimizer: bool = False):
    """Save Keras model as legacy .h5 (only if TF SavedModel causes problems)."""
    file_util.maybe_make_dirs(filename)
    with tempfile.NamedTemporaryFile(suffix=".h5") as tmp:
        tf.keras.models.save_model(model, tmp.name,
                                   include_optimizer=include_optimizer)
        gfile.copy(tmp.name, filename, overwrite=True)


def save_dict_to_json(data: Dict[str, Any], path: Text):
    """Serialize a dictionary to JSON on disk."""
    file_util.maybe_make_dirs(path)
