...
Integrated LiteUNetSelectiveDCT (+ optional bottleneck Transformer) for DCT ablation experiments
"""
...
# >>> IMPORTANT: import ONLY the DCT model you actually use <<<
# (this should be the file that contains LiteUNetSelectiveDCT + bottleneck transformer)
from light_unet_dct_selective_optional_compression2 import LiteUNetSelectiveDCT
...
class HParams:
    ...
    compression_ratio = 0.5           # 1.0=no compression; 0.5=keep 50% LF block
    threshold_mode = "soft"           # "soft" or "hard"
    learnable_transform = False       # learnable DCT matrices? usually False

    # >>> NEW: Bottleneck Transformer hyperparameters (must match your model signature) <<<
...
def evaluate_model(model, dataset, hp, threshold=0.5):
    ...
    Returns: mean_iou, roc_auc, precision, recall, pr_auc, f1
    """
    ...
    pr_auc = compute_pr_auc(all_y_true, all_y_pred)
...
INIT_CKPT_METRIC = "best_pr_auc"   # "best_pr_auc" | "best_iou" | "best_f1"
...
EXPERIMENT_ROOT = "gaussian_sweep_results_Full_DCT_Unet_0.5CP"
...
for profile in ["fine"]:
    for combine_mode in COMBINE_MODES:
        ...
        # Model
        # -----------------------
        print("\n===== DCT Ablation Configuration =====")
        for k, v in hp.use_dct.items():
            print(f"  {k}: {'DCT2D' if v else 'Conv2D'}")
        print(f"  compression_ratio: {hp.compression_ratio}")
        print(f"  threshold_mode: {hp.threshold_mode}")
        print(f"  learnable_transform: {hp.learnable_transform}")
        print(f"  use_bottleneck_transformer: {hp.use_bottleneck_transformer}")
        if hp.use_bottleneck_transformer:
            print(f"  trans_layers={hp.trans_layers}, heads={hp.trans_heads}, dim_head={hp.trans_dim_head}, "
                  f"mlp_ratio={hp.trans_mlp_ratio}, dropout={hp.trans_dropout}")
        print("======================================\n")
        ...
        model = LiteUNetSelectiveDCT(
            ...
            base_c=4,
            use_dct=hp.use_dct,
            compression_ratio=hp.compression_ratio,
        ...
        if init_ckpt_path is not None:
            ...
            state_dict = torch.load(init_ckpt_path, map_location=device)
        ...
        # -----------------------
        # Save model summary + config
        # -----------------------
        summary_path = os.path.join(save_dir, "model_summary.txt")
        with open(summary_path, "w") as f:
            with redirect_stdout(f):
                print(f"LiteUNetSelectiveDCT | sigma-profile={profile} | combine={combine_mode}")
                total_params = sum(p.numel() for p in model.parameters())
                trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
                print(f"\nTotal params: {total_params:,}")
                print(f"Trainable params: {trainable_params:,}")

        # IMPORTANT FIX: vars(hp) won't capture class attributes reliably.
        hp_dict = {k: getattr(hp, k) for k in dir(hp)
                   if not k.startswith("_") and not callable(getattr(hp, k))}
        ...
        for metric_name, ckpt_name in [
            ...
            if os.path.exists(ckpt_path):
                ...
                iou_t, auc_t, prec_t, rec_t, pr_auc_t, f1_t = evaluate_model(model, test_ds, hp, best_thr)
        ...
        test_report = dict(
            ...
            best_metrics=dict(pr_auc=best_metrics_pr, iou=best_metrics_iou, f1=best_metrics_f1),
        ...
        print(f"   Best (validation) → PR-AUC={best_pr_auc:.4f} | IoU={best_iou:.4f} | F1={best_f1:.4f}")
        for metric_name, result in test_results.items():
            print(f"   [{metric_name}] Test → PR-AUC={result['test_pr_auc']:.4f} | IoU={result['test_iou']:.4f} | F1={result['test_f1']:.4f}")
        print("✅  Saved model summary, config, and metrics for "
